...
arbol = tree.DecisionTreeClassifier(random_state=0, max_depth=5) #podemos limitar a profundidad 5 para generar un árbol legible aunque pierda algo de precisión
...
clf = RandomForestClassifier(n_estimators=200, max_depth=None, min_samples_split=5, random_state=0)
clf, y_test_clf, y_prob_clf = validacion_cruzada(clf,X,y,skf)
curva_ROC(3,False,y_test_clf,y_prob_clf,'RF')
#'''

'''
Visualizar las características más importantes según la frecuencia con que se usan en los árboles del ensemble learner (sobre el último modelo de la CV)
'''
