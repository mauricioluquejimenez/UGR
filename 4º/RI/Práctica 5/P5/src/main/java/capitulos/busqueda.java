    ...
    public void indexSearch(Analyzer a, Similarity s) throws IOException, ParseException{
        ...
        for(ScoreDoc i : sd){
            ...
            //searcher.explain(q,i.doc.);
             ...
             float imdb_rating = doc.getField("imdb_rating").numericValue().floatValue();
             int imdb_votes = doc.getField("imdb_votes").numericValue().intValue();
             int number_in_season = doc.getField("number_in_season").numericValue().intValue();
             //int air_date = doc.getField("original_aire_date").numericValue().intValue();
             int air_year = doc.getField("original_aire_year").numericValue().intValue();
             int season = doc.getField("season").numericValue().intValue();
             String title = doc.get("title");
             float viewers = doc.getField("us_viewers_in_million").numericValue().floatValue();
             int views = doc.getField("views").numericValue().intValue();
