...
# Función para verificar si una configuración es resoluble
def es_resoluble(estado):
    # Contar el número de inversiones
    inversiones = 0
    estado_sin_cero = [int(c) for c in estado if c != '0']
    for i in range(len(estado_sin_cero)):
        for j in range(i + 1, len(estado_sin_cero)):
            if estado_sin_cero[i] > estado_sin_cero[j]:
                inversiones += 1

    # Determinar la fila del espacio vacío (0)
    fila_vacio = estado.index('0') // 3

    # Regla de solubilidad
    if fila_vacio % 2 == 0:  # Fila impar (0 o 2)
...
def main():
    if len(sys.argv) != 2:
        print("Uso: python 8_puzzle.py <configuracion>")
        sys.exit(1)
