/* g++ -std=c++11 -pthread -o prodcons_mu_FIFO prodcons_mu_FIFO.cpp scd.cpp */
...
int producir_dato()
{
   ...
   const unsigned valor_producido = siguiente_dato ;

   /* Simplemente se incrementa el siguiente dato a producir */
   siguiente_dato ++;

   mtx.lock();
   cout << "Productor produce: " << valor_producido << endl << flush ;
   mtx.unlock();
...
void consumir_dato( unsigned valor_consumir )
{
   ...
   mtx.lock();
   cout << "                  Consumidor consume: " << valor_consumir << endl ;
   mtx.unlock();
}
//----------------------------------------------------------------------
...
{
   cout << "----------------------------------------------------------------------------" << endl
        << "Problema del productor-consumidor (Monitor SU, buffer FIFO). " << endl
        << "----------------------------------------------------------------------------" << endl
        << flush ;
